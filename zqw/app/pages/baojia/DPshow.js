...
const http_url = 'http://xuanche.17350.com/api/offer/getDPshow';
class DPshow extends Component {
  constructor(props){
    super(props);
    this.state = {
      page: 1,  //当前页
  ...
  static navigationOptions = ({ navigation }) => {
    return {
      title: navigation.state.params.carname ? navigation.state.params.carname:"底盘信息",
      headerStyle: {
        backgroundColor: '#333',
        height:45+Bar,
        paddingTop:Bar
  ...
  fetchData(formData) {
    formData.append('carname',this.props.navigation.getParam('carname'));
    formData.append('cartypeid',this.props.navigation.getParam('cartypeid'));
    ...
    .then((responseData) => {
        //地址
        if(responseData.dpxlname){
          this.setState({
            dpxlname: responseData.dpxlname
          })
        }
        if(responseData.qt){
          this.setState({
  ...
  _renderItemView = ({item,index}) => {
      return (
        <TouchableHighlight 
          onPress={() => this.props.navigation.navigate('dpview',{
              dpxh:item.dpxh,
            })}
          underlayColor='rgba(255, 255, 255, 0)'
        >
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={[styles.item,{marginTop:10}]}>
          <Text style={styles.leftText}>底盘型号</Text>
